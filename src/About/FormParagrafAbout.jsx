...
export const FormParagrafAbout = () => {
  ...
  useEffect(() => {
       if(id){
        axios.get(`http://localhost:5000/ParagrafAbout/${id}`)
  ...
  const HandleForm = async (e) => {
    ...
    if (ParagrafAbout.id) {
      await axios.put(`http://localhost:5000/ParagrafAbout/${ParagrafAbout.id}`, mydata);
    } else {
      await axios.post(`http://localhost:5000/ParagrafAbout`, mydata);
    }
